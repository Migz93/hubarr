...
| Mobile navigation closes from its backdrop or Escape and returns focus to its trigger | Verifies the closed small-screen drawer is inert, then verifies opening it sets initial focus and either dismissal path restores the menu-button focus |
...
| Edit modal traps focus and restores the trigger after Escape | Opens the edit modal, verifies Tab wraps within it, then verifies Escape returns focus to the edit trigger |
