...
| `/opt/hubarr` bind mount | On the Docker host | **Absent — never create it** |
...
Everything for Hubarr lives under a single directory on the host:
...
`hubarr-frontend`, `hubarr-api` etc.
...
Map `/opt/hubarr` on the host to `/config` inside the container (the app
...
Your workspace is mounted at `/workspaces/hubarr` inside the devcontainer.
This is the same directory as `app/` on the host at
`/opt/vscode/node/hubarr/app/`.

## Port

Hubarr runs on port **9301**. Always map `9301:9301` — do not
...
Standard rebuild sequence (run from `/workspaces/hubarr`):
...
Hubarr is worked on by two AI agents — Claude and Codex — usually in separate
...
- A doc whose first line carries a `shared: content` marker is kept identical
  across pacearr and shelfbridge — change it in all of them, or not at all
