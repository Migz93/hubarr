...
Your workspace is mounted at `/workspaces/hubarr` inside this container. This
is the same directory as `app/` on the host at `/opt/vscode/node/hubarr/app/`.
...
- Call out explicitly if the change affects: release behaviour, Docker
  publishing, auth, database schema, Plex/Seerr integrations, or user-visible
  setup
